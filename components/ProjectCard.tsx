  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)]">
      ...
      >
        ...
        {/* Dark hover overlay */}
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/70 transition-all duration-400 flex flex-col justify-end p-5 gap-3">
          ...
          <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-all duration-300 delay-100 translate-y-2 group-hover:translate-y-0">
            {project.url && (
              <a
                ...
                className="flex items-center gap-1.5 text-xs font-mono tracking-widest uppercase text-white border border-white/30 hover:border-white hover:bg-white hover:text-black px-3 py-1.5 rounded transition-colors"
            ...
            {project.github && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1.5 text-xs font-mono tracking-widest uppercase text-white/70 hover:text-white transition-colors"
                onClick={(e) => e.stopPropagation()}
              >
      ...
      <div className="flex items-start justify-between gap-3 px-5 py-4">
        <div className="min-w-0">
          {/* Project name */}
          <h3 className="font-display font-bold text-sm tracking-tight text-[var(--text)] truncate">
            {project.name}
          </h3>
        ...
        <div className="flex flex-col items-end gap-2 shrink-0">
          ...
          {project.url && (
            <a
              ...
              className="p-1.5 rounded border border-[var(--border)] text-[var(--text-subtle)] hover:text-[var(--text)] hover:border-[var(--text-muted)] transition-colors"
            >
